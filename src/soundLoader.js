...
// Extend the Image prototype (aka augment the "class") with my asyncLoad wrapper.
//
...
Audio.prototype.asyncLoad = function(src, asyncCallback) {
    ...
    // NB: The load operation can be triggered from any point after setting `this.src`.
    //
...
// imagePreload
//
// Horrible stuff to deal with the asynchronous nature of image-loading in the browser...
//
...
//
function audioPreload(requiredImages,
                       loadedImages,
                       completionCallback) {
    ...
    // The "for..in" construct "iterates over the enumerable properties of an object,
    /// in arbitrary order." - unlike `Object.keys`, it traverses the prototype chain
